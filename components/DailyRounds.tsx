...
const DailyRounds: React.FC<DailyRoundsProps> = ({ viewDate, setViewDate }) => {
    ...
    const { animals, addSiteLog, addIncident, siteLogs, log_entries } = useAppData();
    ...
    useEffect(() => {
        const existingLog = (siteLogs || []).find((log: SiteLogEntry) => 
    ...
    const categoryAnimals = useMemo(() => {
        if (!animals) return [];
        return animals.filter((a: Animal) => !a.archived && a.category === activeTab);
    }, [animals, activeTab]);

    const totalAnimals = categoryAnimals.length;
    const completedChecks = categoryAnimals.reduce((acc: number, animal: Animal) => {
    ...
    const isNoteRequired = useMemo(() => {
        ...
        const hasUnwateredBird = categoryAnimals.some((a: Animal) => {
            const c = checks[a.id];
    ...
    const handleSignOff = () => {
        ...
        categoryAnimals.forEach((animal: Animal) => {
            const check = checks[animal.id];
            if (!check) return;
        ...
        addSiteLog({
            ...
            description: JSON.stringify({
                ...
                totalChecked: totalAnimals, issuesFound: categoryAnimals.filter((a: Animal) => checks[a.id]?.isAlive === false || checks[a.id]?.securityIssue).length,
                notes: generalNotes, details: checks
            }),
            location: `${activeTab} Section`, priority: 'Medium', status: 'Completed',
        });
    ...
    return (
        <div className="flex flex-col min-h-full bg-slate-100 animate-in fade-in duration-300">
                        ...
                        return (
                            <div key={animal.id} className={`bg-white border-2 rounded-xl p-2 md:p-3 flex items-center gap-2 md:gap-4 transition-all ${isDone ? 'border-emerald-100 shadow-sm' : (state.isAlive === false || state.securityIssue) ? 'border-rose-100 bg-rose-50' : 'border-slate-200'}`}>
                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                    <img src={animal.image_url} alt={animal.name} className="hidden md:block w-12 h-12 rounded-lg object-cover bg-slate-200 shadow-sm shrink-0" />
            ...
            {reportModalOpen && (<div className="fixed inset-0 bg-slate-900/80 z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200"><div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95"><div className={`p-4 border-b flex items-center gap-3 ${reportType === 'HEALTH' ? 'bg-rose-50 border-rose-100' : 'bg-orange-50 border-orange-100'}`}>{reportType === 'HEALTH' ? <AlertTriangle className="text-rose-600" size={24} /> : <Lock className="text-orange-600" size={24}/>}<h2 className={`font-black text-lg uppercase tracking-tight ${reportType === 'HEALTH' ? 'text-rose-900' : 'text-orange-900'}`}>{reportType === 'HEALTH' ? 'Report Health Issue' : 'Aviary Security Alert'}</h2></div><div className="p-6 space-y-4"><p className="text-sm font-medium text-slate-600">You are flagging <strong>{animals.find((a: Animal) => a.id === reportAnimalId)?.name}</strong>. {reportType === 'SECURITY' ? ' Please describe the security or maintenance fault preventing secure lock-up.' : ' Please describe the health observation.'}</p><div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Mandatory Report Notes</label><textarea autoFocus value={issueText} onChange={(e) => setIssueText(e.target.value)} className="w-full bg-slate-50 border-2 border-slate-200 rounded-xl p-3 text-sm font-medium h-32 resize-none focus:border-slate-400 focus:outline-none" placeholder="Details required..."/></div><div className="flex gap-3"><button onClick={() => setReportModalOpen(false)} className="flex-1 py-3 bg-white border-2 border-slate-200 rounded-xl font-bold text-slate-500 hover:bg-slate-50">Cancel</button><button onClick={confirmIssue} disabled={!issueText} className={`flex-1 py-3 text-white rounded-xl font-bold shadow-lg disabled:opacity-50 ${reportType === 'HEALTH' ? 'bg-rose-600 hover:bg-rose-700 shadow-rose-200' : 'bg-orange-600 hover:bg-orange-700 shadow-orange-200'}`}>Confirm Issue</button></div></div></div></div>)}
        </div>
    );
};
