...
const Dashboard: React.FC<DashboardProps> = ({ 
  ...
  const reorderAnimals = async (newOrder: Animal[]) => {
      // Update the 'display_order' property on each animal and persist
      try {
  ...
  const animalStats = useLiveQuery(async () => {
      ...
      // Fetch all logs for today efficiently using the log_date index
      ...
      for (const animal of safeAnimals) {
          ...
          // Fetch previous weights efficiently using the animal_id index
  ...
  }, [animals, viewDate]) || { total: (animals || []).length, weighed: 0, fed: 0, animalData: new Map() };
  ...
  const getWeightDisplay = (log?: LogEntry, unit: 'g' | 'oz' | 'lbs_oz' = 'g') => log ? (log.weight_grams ? formatWeightDisplay(log.weight_grams, unit) : log.value) : '-';
  const getDateDisplay = (log?: LogEntry) => log ? log.log_date.toLocaleDateString('en-GB', { day: 'numeric', month: 'short' }).toUpperCase() : '';
  const getTimeDisplay = (log?: LogEntry) => log ? log.log_date.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '';
  
  return (
    <div className="p-3 md:p-8 pb-40 space-y-4 md:space-y-6 animate-in fade-in duration-500 w-full mx-auto relative z-10">
                          ...
                          return (
                              ...
                              >
                                  ...
                                  <td className="hidden lg:table-cell px-6 py-4 border-b border-slate-100">
                                      <span className="text-[10px] font-black px-3 py-1 rounded-lg border-2 border-slate-100 bg-slate-50 text-slate-500 italic tracking-wider whitespace-nowrap">{animal.latin_name || '-'}</span>
                                  </td>
                                  ...
                                  <td className="hidden lg:table-cell px-6 py-4 border-b border-slate-100">
                                      {d?.previousWeight ? (
                                          <div className="flex flex-col gap-0.5">
                                              <span className="font-black text-slate-500 text-[11px] tracking-tight">{getWeightDisplay(d.previousWeight, animal.weight_unit)}</span>
                                  ...
                                  <td className="hidden lg:table-cell px-6 py-4 border-b border-slate-100">
                                      {(animal.flying_weight_g || animal.winter_weight_g) ? (
                                          <div className="flex flex-col gap-1 leading-none">
                                              {animal.flying_weight_g && <span className="text-[10px] font-black text-blue-600 uppercase tracking-tighter">FLT: {formatWeightDisplay(animal.flying_weight_g, animal.weight_unit)}</span>}
                                              {animal.winter_weight_g && <span className="text-[10px] font-black text-amber-600 uppercase tracking-tighter">WIN: {formatWeightDisplay(animal.winter_weight_g, animal.weight_unit)}</span>}
                                          </div>
